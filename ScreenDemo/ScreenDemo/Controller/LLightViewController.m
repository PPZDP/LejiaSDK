...
- (void)viewDidLoad {
    ...
    if (![LejiaProjectManager sharedManager].isConnectTcpInfo) {
        NSString *msg = @"please LejiaProjectManager method start or reset";
...
-(void)getLightaction
{
    ...
    [[LejiaProjectManager sharedManager] getBrightLevel:^(NSString *lightStr) {
...
-(void)deleteLightaction
{
    ...
    [[LejiaProjectManager sharedManager] setBrightLevel:[self.subLabLight.text integerValue]-1 brightLevelBlock:^(NSString *lightStr) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.subLabLight.text = lightStr;
        });
...
-(void)getUnitaction
{
    [[LejiaProjectManager sharedManager] getSpeedunitStr:^(NSString *unitStr) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)setUnitaction
{
    ...
    [[LejiaProjectManager sharedManager] setSpeedunitStr:unitStr speedUnitBlock:^(NSString *unitStr){
        dispatch_async(dispatch_get_main_queue(), ^{
            self.subLabUnit.text = unitStr;
        });
